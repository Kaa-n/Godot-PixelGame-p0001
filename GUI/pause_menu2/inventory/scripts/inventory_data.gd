...
func add_item(item : ItemData, count : int = 1) -> bool:
	...
	for i in slots.size():
		if slots[i] == null:
			#membuat SlotData baru
			var new = SlotData.new()
			new.item_data = item
			new.quantity = count
			#menambahkan SlotData baru tersebut pada Inventory Data ke-i
			slots[i] = new
...
#mengambil kamus dan mengubahnya menjadi slot data
